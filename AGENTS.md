...
- For opencode-termux branch maintenance, keep fork automation on `dev`, minimal upstream-reviewable Termux source changes on `termux-target`, and an exact mirror of `anomalyco/opencode:dev` on `upstream-dev`. Release and checkpoint workflows must default back to `termux-target`, not the automation branch.
- When rebuilding `termux-target`, start from the current upstream mirror and apply only source/runtime compatibility paths with a 3-way patch or worktree flow. Exclude fork automation, `.github` release workflows, release helper scripts, `.codex` learning files, AGENTS changes, and downloaded artifacts from the PR-ready compatibility branch.
